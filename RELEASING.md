...
1. Update data (if needed)
    - If data changed, run `npm run generate-data` to update `src/data-version.ts` and `data/*.json`.
    - `DATA_VERSION` can pin the date (used in CI to avoid auto-bumping).

2. Update docs
    - Add entries to `CHANGELOG.md` under `[Unreleased]`, then move them to a dated section.
